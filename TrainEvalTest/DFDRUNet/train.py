...
def train(
    ...
    log_dir = f"./Runs/DFDRUNet/{datetime.now().strftime('%Y%m%d_%H%M')[2:]}_{title}/"
    # Dataset & DataLoader
    ...
    if load_weights is not None:
        loadModels(load_weights, DFDRUNet=model)
    ...
    with ProgressManager(len(dataset), epochs, 5, 2, ["Loss", "lr"]) as progress:
        for e in range(epochs):
            ...
            # Save models
            saveModels(os.path.join(log_dir, "last.pth"), DFDRUNet=model)
            if total_loss < best_loss:
                best_loss = total_loss
                saveModels(os.path.join(log_dir, "best.pth"), DFDRUNet=model)

...
def train():
    ...
    # Models
    encoder = Encoder(traj_len=L_TRAJ, encode_c=D_TRAJ_ENC, N_trajs=N_TRAJS).to(DEVICE)
    ...
    with ProgressManager(len(dataloader), EPOCHS, 5, 2, ["loss", "lr"]) as progress:
        for e in range(EPOCHS):
            ...
            for i, batch in enumerate(dataloader):
                ...
                noisy_batch_nodes = ddpm.diffusionForward(batch["nodes"], t, node_noise)
                noisy_batch_adj_mat = ddpm.diffusionForward(batch["adj_mats"], t, adj_mat_noise)

                s = t - 1
                less_noisy_batch_nodes = torch.zeros_like(noisy_batch_nodes)
                less_noisy_batch_nodes[t!=0] = ddpm.diffusionForward(batch["nodes"][t!=0], s[t!=0], node_noise[t!=0])
                less_noisy_batch_nodes[t==0] = batch["nodes"][t==0]

                less_noisy_batch_adj_mat = torch.zeros_like(noisy_batch_adj_mat)
                less_noisy_batch_adj_mat[t!=0] = ddpm.diffusionForward(batch["adj_mats"][t!=0], s[t!=0], adj_mat_noise[t!=0])
                less_noisy_batch_adj_mat[t==0] = batch["adj_mats"][t==0]

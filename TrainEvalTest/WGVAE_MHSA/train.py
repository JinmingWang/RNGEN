...
from Models import WGVAE_MHSA, ClusterLoss, KLLoss
...
def train(
    ...
    log_dir = f"./Runs/WGVAE_MHSA/{datetime.now().strftime('%Y%m%d_%H%M')[2:]}_{title}/"
    ...
    vae = WGVAE_MHSA(N_routes=dataset.N_trajs, L_route=dataset.max_L_route, N_interp=dataset.N_interp, threshold=0.5).to(DEVICE)

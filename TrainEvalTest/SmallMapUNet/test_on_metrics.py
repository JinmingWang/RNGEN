...
def test():
    ...
    titles = product(titles, ["mean", "std"])
    ...
    for batch in tqdm(dataset, desc="Testing"):
        ...
        if pred_segs is None:
            scores.append(reportAllMetrics(pred_2, batch["target_heatmaps"], torch.zeors_like(norm_segs), norm_segs))
            continue

        scores.append(reportAllMetrics(pred_2, batch["target_heatmaps"], pred_segs, norm_segs))
    ...
    if not os.path.exists("Report.csv"):
        with open("Report.csv", "w") as f:
            f.write("name,")
            f.write(",".join([f"{t[0]}_{t[1]}" for t in titles]) + "\n")

    with open("Report.csv", "a") as f:
        f.write("SmallMap,")
        f.write(",".join([f"{m}" for m in mean_scores]) + "," + ",".join([f"{s}" for s in std_scores]) + "\n")

...
def train():
    ...
    # Models
    encoder = TrajAutoEncoder(traj_len=TRAJ_LEN, encode_c=DIM_TRAJ_ENC, N_trajs=N_TRAJS, include_decoder=False).to(DEVICE)
    diffusion_net = DiffusionNetwork(num_nodes=N_NODES, traj_encoding_c=DIM_TRAJ_ENC, traj_num=N_TRAJS, T=T).to(DEVICE)
    if CHECKPOINT is not None:
        encoder, diffusion_net = loadModels(CHECKPOINT, encoder, diffusion_net)
    ...
    for e in range(EPOCHS):
        ...
        for batch_traj, batch_path, batch_graph, batch_heatmap in pbar:
            ...
            # (B, N_nodes, 3), (B, N_nodes, N_nodes)
            batch_nodes, batch_adj_mat = LaDeCachedDataset.SegmentsToNodesAdj(batch_graph, N_NODES)
        ...
        figure = eval(batch_nodes, batch_adj_mat, batch_traj, encoder, diffusion_net, ddpm)
...
def eval(batch_nodes, batch_adj_mat, batch_traj, encoder, diffusion_net, ddpm):
    encoder.eval()
    diffusion_net.eval()
